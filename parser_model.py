...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        nn.init.xavier_uniform_(self.embed_to_hidden.weight, 1)
        self.dropout = nn.Dropout(self.dropout_prob)
        self.hidden_to_logits = nn.Linear(self.hidden_size, self.n_classes)
        nn.init.xavier_uniform_(self.hidden_to_logits.weight, 1)
        ### END YOUR CODE
    ...
    def forward(self, t):
        ...
        x_num_examples = x.shape[0]
        x = x.contiguous().view(x_num_examples, -1)
        logits = self.hidden_to_logits(self.dropout(nn.functional.relu(self.embed_to_hidden(x))))
        # h_lin = self.embed_to_hidden(x)
        # h_relu = nn.functional.relu(h_lin)
